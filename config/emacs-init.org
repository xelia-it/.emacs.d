...
#+BEGIN_SRC emacs-lisp :tangle yes
  (defun my-consult-line-with-symbol-at-point ()
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (use-package consult
    :ensure t
    :defer t
    :bind (("C-f" . my-consult-line-with-symbol-at-point)
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (use-package treesit
    ;; Tree-sitter integrated in Emacs 29+
    :ensure nil
    :config
    ;; Official sources fopr grammars
    ...
    ;; Emacs 30 do not provide treesitter for SCSS
    ;; Add definition for scss-ts-mode derived from css-ts-mode
    (when (treesit-language-available-p 'scss)
      (define-derived-mode scss-ts-mode css-ts-mode "SCSS[TS]"
        "Major mode for editing SCSS files using tree-sitter."
        (treesit-parser-create 'scss))
      (add-to-list 'auto-mode-alist '("\\.scss\\'" . scss-ts-mode)))

    (add-hook 'scss-ts-mode-hook
              (lambda ()
    ...
    ;; Installa automaticamente le grammatiche mancanti
    ;; TODO: add (bash c rust) when grammars are compatible with Emacs 30
    (dolist (lang '(cpp css dockerfile go html javascript json php python ruby scss typescript tsx))
      (unless (treesit-language-available-p lang)
        (ignore-errors
          (treesit-install-language-grammar lang))))

    ;; Auto remap to new "-ts-mode"
    ;; TODO: add (bash c rust) when grammars are compatible with Emacs 30
