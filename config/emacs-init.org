...
#+BEGIN_SRC emacs-lisp :tangle yes
  (use-package shackle
    ...
    (setq shackle-rules '(
                          ...
                          ("\\`\\*bm-bookmarks.*?\\*\\'" :regexp t :align 'below :size 0.25)
                          ("\\`\\*Flymake.*?\\*\\'" :regexp t :align 'below :size 0.25)
                          ("\\`\\*Flycheck.*?\\*\\'" :regexp t :align 'below :size 0.25)
                          ("\\`\\*terminal.*?\\*\\'" :regexp t :align 'below :size 0.25)
                          ))
    (shackle-mode 1)
    )
#+END_SRC
...
It's customized to mimic VS Code behaviour using functions.

#+BEGIN_SRC emacs-lisp :tangle yes
  (defun my/helm-projectile-imenu ()
    "In a project, C-t use helm-imenu to search symbols."
    (interactive)
...
Now we activate Helm. Please note that the loading is deferred in order to speed up startup time.
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (defun my/helm-projectile-or-find-file ()
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (use-package helm-projectile
    ...
    :bind (
           ...
           ("C-p" . my/helm-projectile-or-find-file)
           ;; Switch all buffers
