...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  (defvar my--last-other-file nil
    "Last opened file.")

  (defun my-cycle-other-file ()
    "Cicle between correlated files."
    (interactive)
    (let* ((file (buffer-file-name))
           (matched (cl-find-if (lambda (x)
                                  (string-match (car x) file))
                                my-other-file-alist)))
      (if (not matched)
          (message "No rule found for %s" file)
        (let* ((alts (cadr matched)) ;; Take internal list
               (base (file-name-sans-extension file))
               ;; Manage .spec.ts

               (base (if (string-match "\\.spec\\'" base)
                         (file-name-sans-extension base)
                       base))
               (next-ext
                (if (and my--last-other-file
                         (string-prefix-p base my--last-other-file))
                    (let* ((last-ext (file-name-extension my--last-other-file t))
                           (pos (cl-position last-ext alts :test #'string=)))
                      (nth (mod (1+ (or pos -1)) (length alts)) alts))
                  (car alts)))
               (target (concat base next-ext)))
          (if (file-exists-p target)
              (progn
                (setq my--last-other-file target)
                (find-file target))
            (message "File %s non found" target))))))
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (use-package consult
    :ensure t
    :defer t
    :bind (("C-f" . my-consult-empty-line-or-symbol)
